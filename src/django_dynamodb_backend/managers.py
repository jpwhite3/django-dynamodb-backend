...
class DynamoDBQuerySet(QuerySet):
    ...
    def _build_condition_from_q(self, q_obj):
        ...
        for child in q_obj.children:
            if isinstance(child, Q):
                ...
                child_condition = self._build_condition_from_q(child)
        ...
        elif q_obj.connector == Q.OR:
            # OR: combine with | operator
            combined = child_conditions[0]
            for cond in child_conditions[1:]:
                combined = combined | cond
        else:
            # AND (default): combine with & operator
            combined = child_conditions[0]
            for cond in child_conditions[1:]:
                combined = combined & cond
    ...
    def exclude(self, *args, **kwargs):
        ...
        for arg in args:
            if isinstance(arg, Q):
                # Build condition from Q object and negate it
                condition = clone._build_condition_from_q(arg)
                if condition is not None:
                    clone._dynamodb_scan_filters.append(Not(condition))
    ...
    def bulk_create(
        ...
        Args:
            ...
            ignore_conflicts: If True, ignore duplicate key errors
